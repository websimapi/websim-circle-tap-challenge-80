...
export class VSManager {
    ...
    updateLobbyPreview(presence) {
        ...
        Object.entries(presence).forEach(([clientId, data]) => {
            // Only show users who are actively seeking (waiting for a round)
            // Users who are disconnected are automatically removed from 'presence' by the library,
            // so iterating over presence entries naturally filters out disconnected users.
            if (data.vs_mode_active && data.status === 'seeking' && clientId !== myId) {
